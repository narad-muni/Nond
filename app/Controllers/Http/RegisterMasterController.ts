    ...
    public async create({ request, response }: HttpContextContract) {
        try {
            ...
            if (exist) {
                ...
                await Database.rawQuery(`create table "register__${StringUtils.sanitizeTableName(payload.name + payload.version)}"(
                ...
                await Database.rawQuery(`create table "rollover__register__${StringUtils.sanitizeTableName(payload.name + payload.version)}"(
                    id integer generated by default as identity primary key not null,
                    client_id int4
                )`);
        ...
        try {
            ...
            for await (const register of payload) {
                if (update_query_columns[register.id]?.length && client_columns[register.id]?.length) {
                    ...
                    await Database.rawQuery(`alter table  "register__${StringUtils.sanitizeTableName(register.name + register.version)}" ${client_columns[register.id]}`);
                    await Database.rawQuery(`
                        update "register__${StringUtils.sanitizeTableName(register.name + register.version)}"
                ...
                await Database.rawQuery(`drop table "rollover__register__${StringUtils.sanitizeTableName(register.name + register.version)}"`);
            };
        ...
        try {
            ...
            if (exist?.id != data.id && exist?.id != null) {
                ...
                if (exist?.id == null) {
                    ...
                    await Database.rawQuery(`alter table "register__${StringUtils.sanitizeTableName(old.name + old.version)}" rename to "register__${StringUtils.sanitizeTableName(data.name + data.version)}"`);

                    //rename rollover table
                    await Database.rawQuery(`alter table "rollover__register__${StringUtils.sanitizeTableName(old.name + old.version)}" rename to "rollover__register__${StringUtils.sanitizeTableName(data.name + data.version)}"`);
                }
    ...
    public async destroy({ request, response }: HttpContextContract) {
        try {
            ...
            for (const register of registers) {
                await Database.rawQuery(`drop table "register__${StringUtils.sanitizeTableName(register.name + register.version)}"`);
